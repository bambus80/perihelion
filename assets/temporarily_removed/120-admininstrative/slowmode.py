...
class SlowmodeCog(commands.Cog):
    ...
    async def slowmode(self, interaction: discord.Interaction, seconds: app_commands.Range[int, 0, 21600]):
        ...
        try:
            if seconds == 0:
                await interaction.response.send_message(embed=embed_template(interaction, "Slowmode disabled", "Slowmode delay set to 0 seconds."))
            await interaction.response.send_message(embed=embed_template(interaction, "Slowmode enabled", f"Slowmode delay set to {seconds} seconds."))
        except Forbidden as err:
            await interaction.response.send_message(embed=error_template(interaction, "Perihelion doesn't have permissions to manage channels!"), ephemeral=True)
