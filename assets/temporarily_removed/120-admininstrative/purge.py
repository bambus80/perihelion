...
class PurgeCommandCog(commands.Cog):
    ...
    async def purge(self, interaction: discord.Interaction, count: app_commands.Range[int, 1, 100]):
        ...
        try:
            ...
            await interaction.response.send_message(embed=embed_template(interaction, "Purged messages", f"{len(deleted)} messages were purged."), ephemeral=True)
        except Forbidden as err:
            await interaction.response.send_message(embed=error_template(interaction, "Perihelion doesn't have permissions to manage messages!"), ephemeral=True)
