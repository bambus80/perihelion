...
class BanCog(commands.Cog):
    ...
    async def ban(self, interaction: discord.Interaction, user: Member, reason: Optional[str]):
        try:
            ...
            await interaction.response.send_message(embed=embed_template(interaction, "Banned user", f"{user.mention} has been banned!"), ephemeral=True)
        except Forbidden as err:
            await interaction.response.send_message(embed=error_template(interaction, "Perihelion doesn't have permissions to ban members, or the user is above Perihelion's role!"), ephemeral=True)
    ...
    async def kick(self, interaction: discord.Interaction, user: Member, reason: Optional[str]):
        try:
            ...
            await interaction.response.send_message(embed=embed_template(interaction, "Kicked user", f"{user.mention} has been kicked!"), ephemeral=True)
        except Forbidden as err:
            await interaction.response.send_message(embed=error_template(interaction, "Perihelion doesn't have permissions to kick members, or the user is above Perihelion's role!"), ephemeral=True)
    ...
    async def timeout(self, interaction: discord.Interaction, user: Member, reason: Optional[str]):
        try:
            ...
            await user.kick(reason=reason)
            await interaction.response.send_message(embed=embed_template(interaction, "Muted user", f"{user.mention} has been timed out!"), ephemeral=True)
        except Forbidden as err:
            await interaction.response.send_message(embed=error_template(interaction, "Perihelion doesn't have permissions to moderate members, or the user is above Perihelion's role!"), ephemeral=True)
